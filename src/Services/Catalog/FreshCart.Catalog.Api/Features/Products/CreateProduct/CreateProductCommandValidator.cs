...
public sealed partial class CreateProductCommandValidator : AbstractValidator<CreateProductCommand>
{
    ...
    public CreateProductCommandValidator()
    {
        ...
        RuleFor(command => command.Sku)
            .NotEmpty()
            .MaximumLength(ProductConstraints.MaxSkuLength)
            .Must(sku => string.IsNullOrEmpty(sku) || SkuFormatRegex().IsMatch(sku))
            .WithMessage("Sku must be uppercase alphanumeric groups separated by single hyphens, for example FC-SW-0001.");
        ...
        RuleFor(command => command.InitialStockQuantity)
            .InclusiveBetween(0, ProductConstraints.MaxInitialStockQuantity);
    ...
    [GeneratedRegex("^[A-Z0-9]+(?:-[A-Z0-9]+)*$", RegexOptions.None, RegexTimeoutMilliseconds)]
