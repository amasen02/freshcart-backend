# Notification service - raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the notification service, exactly as
the Helm chart at `deploy/helm/notification/` would render them. They are kept here as a teaching
...
```bash
helm upgrade --install notification deploy/helm/notification \
  --namespace freshcart --create-namespace \
  --values deploy/helm/notification/values-prod.yaml
```
