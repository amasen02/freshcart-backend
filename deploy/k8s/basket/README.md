# Basket service - raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the basket service, exactly as
the Helm chart at `deploy/helm/basket/` would render them. They are kept here as a teaching
...
```bash
helm upgrade --install basket deploy/helm/basket \
  --namespace freshcart --create-namespace \
  --values deploy/helm/basket/values-prod.yaml
```
