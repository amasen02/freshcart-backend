# Customersupport service - raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the customersupport service, exactly as
the Helm chart at `deploy/helm/customersupport/` would render them. They are kept here as a teaching
...
```bash
helm upgrade --install customersupport deploy/helm/customersupport \
  --namespace freshcart --create-namespace \
  --values deploy/helm/customersupport/values-prod.yaml
```
