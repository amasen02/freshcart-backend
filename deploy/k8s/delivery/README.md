# Delivery service - raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the delivery service, exactly as
the Helm chart at `deploy/helm/delivery/` would render them. They are kept here as a teaching
...
```bash
helm upgrade --install delivery deploy/helm/delivery \
  --namespace freshcart --create-namespace \
  --values deploy/helm/delivery/values-prod.yaml
```
