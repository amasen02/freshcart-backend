# Inventory service - raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the inventory service, exactly as
the Helm chart at `deploy/helm/inventory/` would render them. They are kept here as a teaching
...
```bash
helm upgrade --install inventory deploy/helm/inventory \
  --namespace freshcart --create-namespace \
  --values deploy/helm/inventory/values-prod.yaml
```
