# Ordering service - raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the ordering service, exactly as
the Helm chart at `deploy/helm/ordering/` would render them. They are kept here as a teaching
...
```bash
helm upgrade --install ordering deploy/helm/ordering \
  --namespace freshcart --create-namespace \
  --values deploy/helm/ordering/values-prod.yaml
```
