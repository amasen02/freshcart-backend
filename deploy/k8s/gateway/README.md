# Gateway service - raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the gateway service, exactly as
the Helm chart at `deploy/helm/gateway/` would render them. They are kept here as a teaching
...
helm upgrade --install gateway deploy/helm/gateway \
  ...
  --values deploy/helm/gateway/values-prod.yaml
```
