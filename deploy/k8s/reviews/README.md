# Reviews service - raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the reviews service, exactly as
the Helm chart at `deploy/helm/reviews/` would render them. They are kept here as a teaching
...
```bash
helm upgrade --install reviews deploy/helm/reviews \
  --namespace freshcart --create-namespace \
  --values deploy/helm/reviews/values-prod.yaml
```
