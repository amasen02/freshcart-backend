# Catalog service - raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the catalog service, exactly as
the Helm chart at `deploy/helm/catalog/` would render them. They are kept here as a teaching
...
```bash
helm upgrade --install catalog deploy/helm/catalog \
  --namespace freshcart --create-namespace \
  --values deploy/helm/catalog/values-prod.yaml
```
