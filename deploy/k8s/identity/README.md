# Identity service — raw Kubernetes manifests (reference)

This folder contains the **expanded** Kubernetes manifests for the Identity service, exactly as
the Helm chart at `deploy/helm/identity/` would render them. They are kept here as a teaching
...
helm upgrade --install identity deploy/helm/identity \
  ...
  --values deploy/helm/identity/values-prod.yaml
```
